...
void printMatrix(double* matrix, size_t r, size_t c) {
    for (size_t row = 0; row < r; ++row) {
        for (size_t col = 0; col < c; ++col) {
            DebugSimplex("%10.3f ", matrix[row * (m + n) + col]);
        }
        DebugSimplex("\n");
    }
}
...
void printLPInfo(double objFuncCoeff[], variable b[], double* matrix) {
    ...
    for (size_t i = 0; i < m + n; ++i) {
        DebugSimplex("%10.3f ", objFuncCoeff[i]);
    }

    DebugSimplex("\nb = ");

    for (size_t i = 0; i < m; ++i) {
        DebugSimplex("%10.3f ", b[i].value);
    }
...
void printVariables(size_t nonbasic[], variable b[]) {
    ...
    for (size_t i = 0; i < n; ++i) {
        DebugSimplex("x%lu ", nonbasic[i] + 1);
    }
...
int main(int argc, const char* argv[]) {
    ...
    while (true) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            DebugSimplex("x%lu %5.3f ", varLabel + 1, cnbar);
        ...
        if (cnbars.size() == 0) {
            ...
            DebugSimplex("Entering variable is x%lu \n", enteringLabel + 1);
        }
        ...
        while (true) {
            ...
            for (size_t row = 0; row < d.size(); ++row) {
                ...
                if (t_row >= 0.0) {
                    DebugSimplex("x%lu %5.3f ", b[row].label + 1, t_row);
                }
        ...
        DebugSimplex("E%lu = column %lu: ", counter, leavingRow);
